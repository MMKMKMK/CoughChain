...
def build_cot_prompt():
    return (
        "你是一名音频分析专家，请判断这段音频是否包含咳嗽声。请按以下推理步骤进行分析：\n\n"
        ...
        "注意：只有当声音同时具备“突发性 + 爆破性 + 呼气特征”，并能排除其他声音类型时，才可以判断为咳嗽。\n\n"
        "请最终只输出以下两种之一：\n"
        "“有咳嗽” 或 “无咳嗽”。"
    )

# 遍历文件夹，批量识别咳嗽（使用 CoT 提示），并实时写入结果
def detect_cough_in_folder(root_dir, output_file="cough_omni-latest_step.txt"):
    # 清空旧文件内容，准备写入新结果
