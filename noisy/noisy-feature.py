...
def build_cot_prompt():
    return (
        "你是一个专业的音频分析助手，请仔细分析以下在嘈杂环境中录制的音频内容，重点排查是否包含被背景噪声掩盖的咳嗽声。\n\n"
        "注意：\n"
        "由于环境存在人声、风声、交通或其他干扰，咳嗽声可能较弱或部分被掩盖，因此你需要像经验丰富的医生听诊一样，集中注意力识别细微但关键的特征。\n\n"
        "咳嗽通常具有以下典型特征：\n"
        ...
        "- 通常孤立存在，不具有语音节奏和语义内容；\n"
        "判断原则：\n"
        "即使声音较弱，只要**多个特征共现且能排除其他解释**，就应判断为‘有咳嗽’；\n"
        "若仅有模糊响动但无明确特征，则判断为‘无咳嗽’。\n\n"
        "请务必仔细排查，不要因环境嘈杂而轻易忽略潜在信号。\n\n"
        "最终，请仅输出以下两种之一：\n"
        "“有咳嗽” 或 “无咳嗽”"
    )

# 遍历文件夹，批量识别咳嗽（使用 CoT 提示），并实时写入结果
def detect_cough_in_folder(root_dir, output_file="audiofeature_noisy3.txt"):
    # 清空旧文件内容，准备写入新结果
