...
def build_cot_prompt():
    return (
        "你是一名音频分析专家，请判断这段在**嘈杂环境**中录制的音频是否包含咳嗽声。请按以下推理步骤进行分析：\n\n"
        ...
        " 注意：\n"
        "不要因为环境嘈杂就忽略微弱信号。只要某个声音事件具备多个咳嗽特征，且能合理排除其他解释，就应判断为‘有咳嗽’；\n"
        "若仅有模糊响动但缺乏明确模式，则判断为‘无咳嗽’。\n\n"
        "请务必像医生听诊一样，在噪声中仔细排查潜在的咳嗽线索。\n\n"
        "请最终只输出以下两种之一：\n"
        "“有咳嗽” 或 “无咳嗽”"
    )

# 遍历文件夹，批量识别咳嗽（使用 CoT 提示），并实时写入结果
def detect_cough_in_folder(root_dir, output_file="step_noisy.txt"):
    # 清空旧文件内容，准备写入新结果
