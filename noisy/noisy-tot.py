...
def build_cot_prompt():
    return (
        "你是一名经验丰富的音频识别专家，请判断以下在**嘈杂环境**中录制的音频是否包含咳嗽声。\n\n"
        ...
        "- 其他情况一律判断为“无咳嗽”。\n\n"
        "注意：不要因环境嘈杂而忽略微弱但模式清晰的信号。只要多个特征共现且干扰可排除，即使音量低也应考虑为咳嗽。\n\n"
        "请最终只输出：“有咳嗽” 或 “无咳嗽”。可简要说明判断依据。"
    )

# 遍历文件夹，批量识别咳嗽（使用 CoT 提示），并实时写入结果
def detect_cough_in_folder(root_dir, output_file="tot_noisy.txt"):
    # 清空旧文件内容，准备写入新结果
