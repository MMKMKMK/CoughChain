...
def build_cot_prompt():
    return (
        "你是一个专业的音频分析助手，请分析以下在**安静环境**中录制的音频内容，并判断是否包含咳嗽声。\n\n"
        "由于环境安静，背景干扰极少，任何符合咳嗽特征的声音都应被重点关注。\n\n"
        "咳嗽通常具有以下典型特征：\n"
        "- 突发性强，常表现为突然出现的短促声音，持续时间通常在0.3到1秒之间；\n"
        "- 音调变化明显，能量集中在中高频段，常伴有‘爆破’或‘摩擦’的音色；\n"
        "- 由呼气主导，能量迅速上升后快速衰减，有较强的气流感；\n"
        "- 通常孤立存在，不具有语音节奏和语义内容；\n"
        "在安静环境中，即使咳嗽较轻微，只要具备上述多个特征，即可判断为‘有咳嗽’。\n"
        "若存在疑点但仍符合部分关键特征，可适度放宽判断标准。\n\n"
        "最终，请仅输出以下两种之一：\n"
        "“有咳嗽” 或 “无咳嗽”"
    )

# 遍历文件夹，批量识别咳嗽（使用 CoT 提示），并实时写入结果
def detect_cough_in_folder(root_dir, output_file="audiofeature_quiet.txt"):
    # 清空旧文件内容，准备写入新结果
